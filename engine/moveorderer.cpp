...
static const int KILLER_SCORE = MAX_MOVE_SCORE - 100;
    ...
    Move* moves, int cmoves)
{
    ...
    for (int mi = 0; mi < cmoves; ++mi) {
        ...
        // 2) killer potezi (samo mirni potezi)
        if ((moves_are_equal(m, killer1) || moves_are_equal(m, killer2))
            && m.promotion == -1) {
            m.score = KILLER_SCORE;
            continue;
        }
        ...
        // 4) Tihi potez (score ostaje 0) — kasnije možeš dodati history itd.
    }
}
